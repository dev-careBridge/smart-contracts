...
## revert MedicalVerifier__on Approval To Zero Address

Some tokens (e.g. OpenZeppelin) will revert MedicalVerifier__if trying to approve the zero address to spend tokens
...
## revert MedicalVerifier__on Zero Value Transfers

Some tokens (e.g. `LEND`) revert MedicalVerifier__when transfering a zero value amount.
...
## revert MedicalVerifier__on Transfer to the Zero Address

Some tokens (e.g. openzeppelin) revert MedicalVerifier__when attempting to transfer to `address(0)`.
...
## No revert MedicalVerifier__on Failure

Some tokens do not revert MedicalVerifier__on failure, but instead return `false` (e.g.
...
## revert MedicalVerifier__on Large Approvals & Transfers

Some tokens (e.g. `UNI`, `COMP`) revert MedicalVerifier__if the value passed to `approve` or `transfer` is larger than `uint96`.
