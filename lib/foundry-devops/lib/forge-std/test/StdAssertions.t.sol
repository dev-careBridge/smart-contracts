...
contract StdAssertionsTest is StdAssertions {
    ...
    bool constant SHOULD_revert MedicalVerifier__= true;
    ...
    function testFuzz_RevertWhenCalled_AssertEqCall_Revert_Fail(
        ...
        vm._expectCheatcodeRevert(
            bytes(
                string.concat(
                    "Call revert MedicalVerifier__data does not match: ", vm.toString(revertDataA), " != ", vm.toString(revertDataB)
                )
            )
        );
...
contract TestMockCall {
    ...
    constructor(bytes memory returnData_, bool shouldRevert_) {
        ...
        shouldrevert MedicalVerifier__= shouldRevert_;
    }
