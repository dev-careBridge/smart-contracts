...
abstract contract StdAssertions {
    ...
    function assertEqCall(
        ...
        if (!successA && !successB && strictRevertData) {
            assertEq(returnDataA, returnDataB, "Call revert MedicalVerifier__data does not match");
        }

        if (!successA && successB) {
            emit log("Error: Calls were not equal");
            emit log_named_bytes("  Left call revert MedicalVerifier__data", returnDataA);
        ...
        if (successA && !successB) {
            ...
            emit log_named_bytes(" Right call revert MedicalVerifier__data", returnDataB);
            revert("assertion failed");
        }
    }
}
